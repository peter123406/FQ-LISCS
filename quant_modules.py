...
class Intsigmoid(nn.Module):
    ...
    def __init__(self, output_bit=8):
        super(Intsigmoid, self).__init__()
    ...
    def cut_input(x,scaling_factor):
        x_detached=x.detach()
        x_detached=x_detached/scaling_factor*(2**16)
        x_detached=torch.round(x_detached).int()+2**23
        temp_num=8/scaling_factor*2**16
        baoliu_bit=math.floor(math.log2(temp_num+1))+1

        threshold_upper = (1 << baoliu_bit) - 1+2**23
        threshold_lower = -(1 << baoliu_bit) +2**23
        diff_upper = x_detached - threshold_upper
        diff_lower = threshold_lower-x_detached  

        mask_upper = (diff_upper >> 31) & 1  
        mask_upper = -mask_upper             
        mask_lower = (diff_lower >> 31) & 1  
        mask_lower = -mask_lower    
        x_detached= (x_detached & mask_upper) | (torch.tensor(threshold_upper).cuda() & ~mask_upper)
        x_detached= (x_detached & mask_lower) | (torch.tensor(threshold_lower).cuda() & ~mask_lower)

        x=(x_detached-2**23)*scaling_factor/2**16

        threshold_bit = 23
        diff_upper_bit=baoliu_bit-threshold_bit 
        mask_upper_bit = (diff_upper_bit >> 31) & 1
        mask_upper_bit = -mask_upper_bit    
    ...
    def forward(self, x, scaling_factor=None):
        ...
        scaling_factor_sig = scaling_factor

        x_int_max, _ = pre_x_int.max(dim=-1, keepdim=True)
        x_int = pre_x_int - x_int_max  # I_DELTA
        ...
        x_int = sigmoid_int
        scaling_factor = sigmoid_scaling_factor
...
class QuantAct(nn.Module):
    ...
    def __init__(self,
        ...
        self.register_buffer('min_val', torch.zeros([]))
        self.register_buffer('max_val', torch.zeros([]))
    ...
    def forward(self, x, pre_act_scaling_factor=None):
        ...
        with torch.no_grad():
            ...
            self.conv_scaling_factor = symmetric_linear_quantization_params(
                self.weight_bit, self.min_val, self.max_val)#s_w

        self.weight_integer = self.weight_function(
            self.weight, self.weight_bit, self.conv_scaling_factor, True)
        bias_scaling_factor = self.conv_scaling_factor * pre_act_scaling_factor#pre=s_x
        self.bias_integer = self.weight_function(
            self.bias, self.bias_bit, bias_scaling_factor, True)
        ...
        S=F.conv2d(x_int, self.weight_integer, self.bias_integer, self.stride, self.padding,
...
class IntLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x, scaling_factor=None):
        ...
        genhaovar_int=torch.sqrt(var_int)
        # Integer Iteration
        k = 2 ** 16
        for _ in range(10):
